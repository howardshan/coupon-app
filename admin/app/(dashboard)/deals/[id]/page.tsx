...
export default async function DealReviewPage({
  ...
  const merchantName = (deal.merchants as { name?: string } | null)?.name ?? '—'
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3 text-sm">
            ...
            <div><dt className="text-gray-500">Merchant</dt><dd className="font-medium text-gray-900">{merchantName}</dd></div>
            ...
            <div className="flex flex-wrap gap-3">
              {(deal.image_urls as string[]).map((url, i) => (
                <a
                  ...
                  className="block w-24 h-24 rounded-lg overflow-hidden border border-gray-200 hover:opacity-90"
                >
