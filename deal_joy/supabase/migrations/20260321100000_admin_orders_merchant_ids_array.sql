-- Admin 订单搜索：商家筛选由单 uuid 改为 uuid[]，支持多选
-- 须先 DROP 再 CREATE（参数列表变更）

DROP FUNCTION IF EXISTS public.get_admin_orders_search(
  text, uuid, text[], timestamptz, timestamptz, numeric, numeric, text, int, int
);

DROP FUNCTION IF EXISTS public.get_admin_orders_count(
  text, uuid, text[], timestamptz, timestamptz, numeric, numeric
);
...
BEGIN
  ...
  AND (
    p_merchant_ids IS NULL
    OR array_length(p_merchant_ids, 1) IS NULL
