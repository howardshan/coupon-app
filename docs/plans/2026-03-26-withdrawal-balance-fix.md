...
**修复后只需更新 `order_items` 一张表。**

`/balance` 和 Earnings RPC 现在统一使用 `order_items.redeemed_at` 作为 T+7 判断字段，不再读取 `coupons.used_at`。

```sql
-- Step 1：查出目标 order_item
SELECT oi.id, oi.redeemed_at
FROM order_items oi
WHERE COALESCE(oi.redeemed_merchant_id, oi.purchased_merchant_id) = '<merchant_id>'
  AND oi.customer_status = 'used'
ORDER BY oi.redeemed_at DESC LIMIT 5;

-- Step 2：将 order_items.redeemed_at 回拨至 8 天前即可
UPDATE order_items
SET redeemed_at = NOW() - INTERVAL '8 days'
WHERE id = '<order_item_id>';
```

> **说明：** 正常业务流程（商家扫码核销）中，`coupons.used_at` 和 `order_items.redeemed_at` 由 `merchant-scan` 用同一个时间戳同步写入，两者始终一致。需要同时更新两张表的情况只出现在修复前的旧测试数据中（旧 `/balance` 读 `coupons.used_at`，Earnings 读 `order_items.redeemed_at`，测试时改一张表另一张不同步）。
